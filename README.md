...
- 前端结构化题面表单编辑：可直接修改标题、描述、输入输出、约束、样例与备注，改动会自动保存。
...
- 前端自动保存与主题切换：题面、用户代码、结构化题面已去掉手动保存按钮，改成自动保存，并新增深浅模式切换。
...
- 直接在题面、用户代码、结构化题面里自动保存，不需要再找保存按钮
- 一键切换深色 / 浅色主题，Monaco 编辑器会同步切换
- 编辑结构化题面
- 把对拍失败输入回填到快速运行面板
