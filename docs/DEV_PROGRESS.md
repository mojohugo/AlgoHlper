...
- 当前已接通这些流程：
  ...
  - Monaco 编辑器接入（题面 / 用户代码 / 产物 / 结构化题面 / 日志）
  - 一键复制（题面 / 代码 / 产物 / 反例输入 / expected / actual / 编译日志）
  - 结构化题面表单化编辑（标题 / 描述 / IO / 约束 / 样例 / 备注）
  - 快速运行面板（当前 user_solution + 任意输入 + 反例回填）
  - 工作台拆分为四个分区（概览 / 编辑 / 生成资产 / 对拍运行），减少单页堆叠
  - 解析 / 生成 / 对拍后自动切换到对应分区，任务日志不再需要手动来回找
  - 状态文案前端人性化（如 `counterexample_found -> 发现反例`），并修复长状态把卡片 / 徽标撑破的问题
  - 前端主要可见文案进一步中文化：项目摘要、运行环境、产物标签、快测输出、对拍结果等统一为中文表达
  - 前端冗余内容收缩：概览页去掉重复运行环境摘要、减少状态卡片数量，项目列表去掉冗余 ID 行
  - 题面 / 用户代码 / 结构化题面改为自动保存，移除手动保存按钮
  - 新增深浅模式切换，页面样式和 Monaco 编辑器主题会同步切换
...
- `POST /api/projects/{project_id}/run-user`
...
- 写了快速运行接口测试，覆盖 `/api/projects/{project_id}/run-user` 的编译执行路径。
- 写了对拍测试，能稳定发现错误程序的反例。
